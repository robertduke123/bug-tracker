			...
			props.projects.forEach((project) => {
				if (project?.tickets) {
					project.tickets.forEach((ticket) => {
						if (
							change ||
							ticket.author === props.user.firstName + " " + props.user.lastName
						) {
	...
	let listItems = props.projects.map((project) => {
		...
		return (
			<li
				...
				<h2 className="project-name" onClick={(e) => props.loadProject(e)}>
					{project.name}
				</h2>
				<p>{project.description}</p>
				<p>{contributors}</p>
				<div className="edit" onClick={(e) => edit(e)}>
					...
				</div>
				<ul className="edit-container closing">
					<li onClick={(e) => props.newFormControl(e, "project", edit)}>
						edit
					</li>
		...
		<div className="dashboard">
			<div className="projects-container">
				<div className="project-head">
					...
					<div
						className="btn"
						onClick={(e) => props.newFormControl(e, "project")}>
						New Project
					</div>
				</div>
				...
				<ul>{props.projects ? listItems : <div></div>}</ul>
			</div>

			<div className="graphs-header">
				<h3>{`${change ? "The Team's" : "Your"} Tickets`}</h3>
				<div
					className="btn"
					style={{ marginTop: "2px" }}
				...
				<div className="graph-container">
					<div className="graph">
						...
						<div className="legend">
							<div
								className="pie-color"
								style={{ background: "#4f7fa9" }}></div>
							<p style={{ color: "#4f7fa9" }}>Issue</p>
							<div
								className="pie-color"
								style={{ background: "#769cbc" }}></div>
							<p style={{ color: "#769cbc" }}>Bug</p>
							<div
								className="pie-color"
								style={{ background: " #9db8cf" }}></div>
							<p style={{ color: " #9db8cf" }}>Feature Request</p>
						</div>
					</div>
					...
					<div className="graph">
						...
						<div className="legend">
							<div
								className="pie-color"
								style={{ background: "#4f7fa9" }}></div>
							<p style={{ color: "#4f7fa9" }}>Low</p>
							<div
								className="pie-color"
								style={{ background: "#769cbc" }}></div>
							<p style={{ color: "#769cbc" }}>Medium</p>
							<div
								className="pie-color"
								style={{ background: " #9db8cf" }}></div>
							<p style={{ color: " #9db8cf" }}>High</p>
							<div
								className="pie-color"
								style={{ background: " #c4d4e2" }}></div>
							<p style={{ color: " #c4d4e2" }}>Immediate</p>
						</div>
					</div>
					...
					<div className="graph">
						...
						<div className="legend">
							<div
								className="pie-color"
								style={{ background: "#4f7fa9" }}></div>
							<p style={{ color: "#4f7fa9" }}>New</p>
							<div
								className="pie-color"
								style={{ background: "#769cbc" }}></div>
							<p style={{ color: "#769cbc" }}>In Progress</p>
							<div
								className="pie-color"
								style={{ background: " #9db8cf" }}></div>
							<p style={{ color: " #9db8cf" }}>Resolved</p>
						</div>
					</div>
