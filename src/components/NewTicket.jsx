	...
	loadticket = () => {
		...
		if (
			...
			if (!desc.value) {
				...
				desc.style.border = "1px solid gray";
			}
			if (!devSelect.value) {
				devSelect.style.border = "1px solid red";
			} else {
				devSelect.style.border = "1px solid gray";
			}
			if (!timeEst.value || isNaN(timeEst.value)) {
				timeEst.style.border = "1px solid red";
			} else {
				timeEst.style.border = "1px solid gray";
			}
		}
	};
			...
			<div className="new-project-container">
				<form className="new-ticket-form">
					<div className="form-header">
						...
						<div
							className="close"
							onClick={(e) => this.props.newFormControl(e, "hidden", false)}>
							x
						</div>
					</div>
					...
					<textarea
						...
						cols="30"
						rows="10"
						onChange={this.ticketDescription}
						defaultValue={
							this.props.edit.state ? this.props.edit.project.description : ""
						}></textarea>
					...
					<div className="form-sec">
						...
						<input
							...
							id="time-estimate"
							placeholder="20"
							onChange={this.ticketTime}
							defaultValue={
								this.props.edit.state ? this.props.edit.project.time : ""
							}
						/>
					</div>
					...
					<div className="form-sec">
						...
						<select
							name="ticket-type"
							id="ticket-type"
							onChange={this.ticketType}>
							<option
								selected={
									this.props.edit.project.type === "issue" ? "selected" : ""
								}>
								Issue
							</option>
							<option
								selected={
									this.props.edit.project.type === "bug" ? "selected" : ""
								}>
								Bug
							</option>
							<option
								selected={
									this.props.edit.project.type === "feature request"
										? "selected"
										: ""
								}>
								Feature Request
						...
						<select
							...
							id="ticket-priority"
							onChange={this.ticketPriority}>
							<option
								selected={
									this.props.edit.project.priority === "low" ? "selected" : ""
								}>
								Low
							</option>
							<option
								selected={
									this.props.edit.project.priority === "medium"
										? "selected"
										: ""
								}>
								Medium
							...
							<option
								selected={
									this.props.edit.project.priority === "high" ? "selected" : ""
								}>
								High
							</option>
							<option
								selected={
									this.props.edit.project.priority === "immediate"
										? "selected"
										: ""
								}>
								Immediate
						...
						<select
							name="ticket-status"
							id="ticket-status"
							onChange={this.ticketStatus}>
							<option
								selected={
									this.props.edit.project.status === "new" ? "selected" : ""
								}>
								New
							</option>
							<option
								selected={
									this.props.edit.project.status === "in progress"
										? "selected"
										: ""
								}>
								In Progress
							...
							<option
								selected={
									this.props.edit.project.status === "resolved"
										? "selected"
										: ""
								}>
								Resolved
