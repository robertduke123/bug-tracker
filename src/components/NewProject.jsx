...
class NewProject extends Component {
	constructor(props) {
		...
		if (this.props.edit.state) {
			this.state = {
				...
				contributors: this.props.edit.project.contributors,
			};
		} else {
			this.state = {
				name: "",
	...
	projectContributors = () => {
		...
		members.forEach((member) => {
			if (member.hasAttribute("selected")) {
				if (!this.state.contributors.includes(member.value)) {
					this.setState((prevState) => ({
						contributors: [...prevState.contributors, member.value],
					}));
				}
			}
		});
	...
	loadProject = () => {
		...
		this.state.contributors.forEach((option) => {
			if (option) {
				opCount++;
			}
		});
			...
			<div className="new-project-container">
				<form className="new-project-form">
					...
					<input
						...
						defaultValue={
							this.props.edit.state ? this.props.edit.project.name : ""
						}
					/>
					...
					<textarea
						...
						cols="30"
						rows="10"
						onChange={this.projectDescription}
						defaultValue={
					...
					<select name="project-members" id="dd" size="4" multiple>
						{this.props.team.map((member) => {
							if (this.props.edit.state) {
								if (
									this.props.edit.project.contributors.includes(
					...
					<div
						className="btn"
						onMouseDown={this.projectContributors}
						onMouseUp={this.loadProject}>
						Submit
					</div>
				</form>
			</div>
